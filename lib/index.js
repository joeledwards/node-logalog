...
function initialize (config = {}) {
  const cfg = {
    mode: config.mode != null ? config.mode : defaultConfig.mode,
    console: config.console != null ? config.console : defaultConfig.console,
    alias: config.alias != null ? config.alias : defaultConfig.alias,
  ...
  const error = (cfg.error != null) ? `${red(cfg.error)}>` : false
  const warn = (cfg.warn != null) ? `${yellow(cfg.warn)}>` : false
  const info = (cfg.info != null) ? `${green(cfg.info)}>` : false
  ...
  function sub (logger, categoryLabel) {
    if (categoryLabel) {
      return (args...) => {
        ...
        const msg = (first != null) ? first : ''
        message = `[${now(cfg.mode)}]${alias}${categoryLabel} ${msg}`
        logger(message, rest...)
      }
    } else {
      return () => {}
